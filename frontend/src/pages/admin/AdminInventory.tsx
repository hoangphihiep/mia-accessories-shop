...
export default function AdminInventory() {
  ...
  // New Receipt State
  const [receiptType, setReceiptType] = useState<'IMPORTED' | 'MANUFACTURED'>('IMPORTED');
  ...
  const fetchData = async () => {
    try {
      ...
      const allVariants = allProducts.flatMap((p: any) =>
        (p.variants || []).map((v: any) => ({ ...v, productName: p.name, productionType: p.productionType }))
      );
  ...
  const handleDetailChange = (index: number, field: string, value: any) => {
    ...
    // Auto-fill unitPrice when a variant is selected for MANUFACTURED receipt
    if (field === 'variantId' && receiptType === 'MANUFACTURED') {
      const selectedVariant = variants.find(v => v.id.toString() === value.toString());
      if (selectedVariant) {
        newDetails[index].unitPrice = selectedVariant.productionCost || 0;
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (receiptType === 'IMPORTED' && !supplierId) {
    ...
    try {
      await api.post('/admin/inventory', {
        type: receiptType,
        supplierId: receiptType === 'IMPORTED' ? parseInt(supplierId) : null,
    ...
    <>
      <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 h-[calc(100vh-9rem)] flex flex-col overflow-hidden">
        <div className="p-8 border-b border-gray-50 flex flex-col md:flex-row items-start md:items-center gap-6 bg-white/50 backdrop-blur-xl shrink-0 z-10 relative">
          <div className="flex-1 flex flex-col md:flex-row items-start md:items-center gap-6 w-full">
            <div className="shrink-0">
              ...
              <p className="text-gray-400 text-sm mt-2 ml-13 whitespace-nowrap">Quản lý và theo dõi các phiếu nhập hàng</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 border-b border-gray-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/50 backdrop-blur-xl shrink-0 z-10 relative">
          <div className="relative w-full md:w-96 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3.5 bg-gray-50 border-none rounded-xl text-sm font-medium focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all placeholder:text-gray-400 outline-none"
            />
          </div>
          ...
          {user?.role?.name === 'ROLE_ADMIN' && (
            <button
              ...
              className="flex items-center justify-center gap-2 bg-gray-900 text-white px-6 py-3 rounded-xl text-sm font-bold uppercase tracking-wider hover:bg-gray-800 hover:shadow-lg hover:-translate-y-0.5 transition-all w-full md:w-auto whitespace-nowrap shrink-0 mt-4 md:mt-0"
            >
              <Plus size={18} />
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/80 text-gray-400 text-[11px] font-black uppercase tracking-[0.2em] sticky top-0 backdrop-blur-sm z-10">
                ...
                <th className="p-5 border-b border-gray-100">Nguồn / Loại phiếu</th>
              ...
              {filteredReceipts.map((receipt) => (
                <tr key={receipt.id} onClick={() => openViewDrawer(receipt)} className="border-b border-gray-50 hover:bg-gray-50/80 transition-colors group cursor-pointer">
                  ...
                  <td className="p-5 font-bold text-gray-900">
                    {receipt.receiptType === 'MANUFACTURED' ? (
                      <span className="px-2.5 py-1 bg-amber-50 text-amber-600 rounded-lg text-[10px] font-black uppercase tracking-wider">Tự sản xuất</span>
                    ) : (
      ...
      {/* Side Drawer */}
  <div
    className={`fixed top-0 right-0 bottom-0 w-full lg:w-[800px] bg-white shadow-2xl border-l border-gray-100 z-50 transform transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col ${showDrawer ? 'translate-x-0' : 'translate-x-[110%]'
      }`}
  >
    <div className="h-20 border-b border-gray-100 flex items-center justify-between px-8 bg-white/50 backdrop-blur-xl shrink-0">
      <h2 className="text-xl font-black uppercase tracking-widest text-gray-900 flex items-center gap-3">
        ...
        {drawerMode === 'create' ? 'Tạo Phiếu Nhập Mới' : 'Chi tiết Phiếu Nhập'}
      </h2>
      <button
        onClick={() => setShowDrawer(false)}
          ...
          <div className="flex gap-6 mb-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                name="receiptType"
                value="IMPORTED"
                checked={receiptType === 'IMPORTED'}
                onChange={() => setReceiptType('IMPORTED')}
                className="w-4 h-4 text-primary focus:ring-primary"
              />
              <span className="text-sm font-bold text-gray-700">Nhập từ NCC</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
                name="receiptType"
                value="MANUFACTURED"
                checked={receiptType === 'MANUFACTURED'}
                onChange={() => {
                  setReceiptType('MANUFACTURED');
                  setSupplierId('');
                }}
                className="w-4 h-4 text-primary focus:ring-primary"
              />
              <span className="text-sm font-bold text-gray-700">Tự sản xuất</span>
            </label>
          </div>

          {receiptType === 'IMPORTED' && (
            <div>
              ...
              {details.map((detail, index) => (
                <div key={index} className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm space-y-3">
                  <div className="flex justify-between items-start gap-2">
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        ...
                        {variants.filter(v => receiptType === 'IMPORTED' || v.productionType === 'MANUFACTURED').map(v => (
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                      <label className="block text-[10px] font-black uppercase text-gray-400 mb-1">{receiptType === 'IMPORTED' ? 'Đơn giá (VNĐ)' : 'Giá vốn (VNĐ)'}</label>
                      <input
                        type="number"
                        min="0"
                        required
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white border border-gray-100 p-4 rounded-xl shadow-sm">
                <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-gray-400 mb-2">
                  <Building2 size={14} /> {selectedReceipt.receiptType === 'MANUFACTURED' ? 'Loại phiếu' : 'Nhà cung cấp'}
                </div>
                <p className="font-bold text-gray-900">
                  {selectedReceipt.receiptType === 'MANUFACTURED' ? (
                    <span className="text-amber-600">Tự sản xuất</span>
                  ) : (
