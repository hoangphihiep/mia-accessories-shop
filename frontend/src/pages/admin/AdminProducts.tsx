                ...
                <div className="space-y-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                  ...
                  <div className="flex gap-4 overflow-x-auto py-2 custom-scrollbar">
                    ...
                    <button 
                      type="button"
                      onClick={() => {
                        const url = window.prompt('Nhập URL hình ảnh (Giả lập tính năng Upload):');
                        if (url) setFormData({...formData, images: [...formData.images, url]});
                      }}
                      className="w-24 h-24 rounded-xl border-2 border-dashed border-gray-300 hover:border-primary flex flex-col items-center justify-center gap-1 text-gray-400 hover:text-primary transition-colors shrink-0 bg-gray-50 hover:bg-sky-50"
                    >
                      <UploadCloud size={20} />
                        ...
                        <div className="flex gap-4">
                          <div className="w-24 shrink-0">
                            {variant.imageUrl ? (
                              ...
                              <button 
                                type="button"
                                onClick={() => {
                                  const url = window.prompt('Nhập URL ảnh cho mẫu này:');
                                  if (url) handleVariantChange(index, 'imageUrl', url);
                                }}
                                className="w-full h-24 rounded-xl border-2 border-dashed border-gray-300 hover:border-primary flex flex-col items-center justify-center gap-1 text-gray-400 hover:text-primary transition-colors bg-gray-50 hover:bg-sky-50"
                              >
                                <ImageIcon size={20} />
                                <span className="text-[9px] font-bold text-center">Thêm ảnh<br/>mẫu mã</span>
                              </button>
                            )}
                          </div>
                          ...
                          <div className="flex-1 space-y-4">
                            ...
                            <div className="flex gap-4">
                              ...
                              <div className="flex-1">
                                <label className="block text-[10px] font-black uppercase text-gray-400 mb-1.5">Giá gốc (Gạch ngang)</label>
                                <input 
                                  type="number" 
                                  min="0"
